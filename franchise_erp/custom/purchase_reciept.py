...
def validate_company_submit(doc, method=None):
    user = frappe.session.user

    # ✅ Administrator can always submit
    if user == "Administrator":
        return

    # ✅ Apply rule only if custom_source_sales_invoice exists
    if not doc.custom_source_sales_invoice:
        return

    # ✅ Get all companies user is allowed to access
    ...
    # Fallback: Default company (just in case)
    if not allowed_companies:
        default_company = frappe.db.get_value("User", user, "company")
        if default_company:
            allowed_companies = [default_company]

    # No company access at all
    if not allowed_companies:
        frappe.throw(_("You are not linked with any Company"))

    # GRN company not in user's allowed companies
    if doc.company not in allowed_companies:
        frappe.throw(_(
            "You are not allowed to submit this GRN. "
            "Only users of company {0} or Administrator can submit."
        ).format(doc.company))
